...
export const addMembers = TryCatch(async (req, res, next) => {
    ...
    return res.status(200).json({
        success: true,
        message: "Member added to this group successfully",
    })
...
export const leaveGroup = TryCatch(async (req, res, next) => {
    ...
    emitEvent(
        ...
        ALERT,
        chat.members,
        {message:`${user} leaved from group`,chatId}
    )
    emitEvent(
        req,
        REFETCH_CHATS,
...
export const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    if(files.length > 5) return next(new ErrorHandler("Files Can't more than 5",400));
...
export const renameGroup = TryCatch(async (req, res, next) => {
    ...
    if (chat.creator.toString() !== req.user.toString())
        return next(new ErrorHandler("You are not allowed to rename the group", 404));
    chat.name = name;
    await chat.save();
...
export const deleteChat = TryCatch(async (req, res, next) => {
    ...
    await Promise.all([
        // Delete files and attachments from cloudinary
